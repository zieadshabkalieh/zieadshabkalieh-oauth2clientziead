...
"""oauth2clientziead Service account credentials class."""
...
class ServiceAccountCredentials(client.AssertionCredentials):
    ...
    * ``.p12`` key (stores PKCS12 key and certificate)
    ...
    def _from_parsed_json_keyfile(cls, keyfile_dict, scopes,
        ...
        if not token_uri:
            token_uri = keyfile_dict.get('token_uri',
                                         oauth2clientziead.GOOGLE_TOKEN_URI)
        if not revoke_uri:
            revoke_uri = keyfile_dict.get('revoke_uri',
                                          oauth2clientziead.GOOGLE_REVOKE_URI)
    ...
    def sign_blob(self, blob):
        ...
        :meth:`oauth2clientziead.client.AssertionCredentials.sign_blob`.
...
class _JWTAccessCredentials(ServiceAccountCredentials):
    ...
    def create_scoped(self, scopes, token_uri=oauth2clientziead.GOOGLE_TOKEN_URI,
                      revoke_uri=oauth2clientziead.GOOGLE_REVOKE_URI):
